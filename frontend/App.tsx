...
type ScreenId =
  | "LANDING" | "I1" | "I2" | "D3" | "D1" | "D2" | "D4" | "D5" | "P1" | "D6"
  | "G1" | "G2" | "G3" | "G4"
...
interface GameState {
  ...
  penalty: { title: string; effect: string } | null;
  cpHistory: CPResult[]; 
  growChoice: string;
  openAcc: string[]; 
  readTools: string[];
...
const INIT_STATE: GameState = {
  ...
  currentStoreIdx: 0,
  checkpoint:1, reqIdx:0, requests:[], results:[], penalty:null, cpHistory:[],
    ...
    <>
      ...
      >
        <button 
          ...
          className={`w-16 h-16 rounded-full bg-primary text-white shadow-2xl flex items-center justify-center hover:scale-105 transition-transform border-4 border-white cursor-grab active:cursor-grabbing ${isOpen ? 'hidden' : 'block'}`}
        >
          <img src="https://api.dicebear.com/8.x/micah/svg?seed=Bubbly&backgroundColor=ff0180" alt="Bubbly" className="w-full h-full rounded-full object-cover pointer-events-none" />
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="bg-primary p-4 flex items-center justify-between text-white">
              <div className="flex items-center gap-3">
                <img src="https://api.dicebear.com/8.x/micah/svg?seed=Bubbly&backgroundColor=ff0180" alt="Bubbly" className="w-10 h-10 rounded-full border-2 border-white object-cover" />
                <div>
    ...
    >
      ...
      <div className={`absolute inset-0 flex flex-col transition-opacity duration-200 ${collapsed ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="p-4 flex flex-col items-center gap-4">
          <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-white font-bold text-xl border-2 border-pink-700">B</div>
      ...
      <div className={`absolute inset-0 flex flex-col transition-opacity duration-200 ${!collapsed ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="p-6 flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-primary/20 border-2 border-pink-700">B</div>
            <div>
...
const ScreenLanding = ({ gs, go, setGs }: ScreenProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full relative">
      <BgIcon Icon={Store} />
      <div className="w-full md:w-1/2 bg-gradient-to-br from-[#0D0D1A] via-[#1a1030] to-[#0D0D1A] p-8 md:p-12 flex flex-col justify-center relative overflow-y-auto">
        ...
        <div className="relative z-10 max-w-lg mx-auto w-full my-auto">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center text-white font-bold text-3xl shadow-2xl shadow-primary/30 border-2 border-pink-700 shrink-0">B</div>
      ...
      <div className="w-full md:w-1/2 bg-white/90 backdrop-blur-sm p-8 md:p-12 flex flex-col justify-center items-center relative z-10 overflow-y-auto">
...
const ScreenD1 = ({ gs, go, setGs }: ScreenProps) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 relative flex flex-col">
      ...
      <BottomNav 
        onBack={() => gs.reopening ? go("D7") : go("D3")} 
        onNext={() => {
          setGs(p => {
            const newStores = [...p.stores];
...
const ScreenP1 = ({ gs, go, setGs }: ScreenProps) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 relative flex flex-col">
      ...
      <BottomNav 
        onBack={() => isMidGame ? go("BSC") : go("D4")} 
        onNext={() => isMidGame ? go("D7") : go("D5")} 
        secondaryLabel={isMidGame ? "Keep Prices As-Is" : undefined}
        onSecondary={isMidGame ? () => go("D7") : undefined}
      />
    </div>
  );
};
